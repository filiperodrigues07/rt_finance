...
export function buildSystemPrompt(ctx: InterpretContext): string {
  const cats = ctx.categories.map((c) => `${c.name} (${c.kind})`).join(", ") || "(nenhuma)";
  const cards = ctx.cards.join(", ") || "(nenhum)";
  const members = ctx.members.join(", ") || "(nenhum)";
  const accounts = ctx.accounts.join(", ") || "(nenhuma)";
